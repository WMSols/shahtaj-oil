...
export class FinancialsInvoicing extends Component {
    ...
    requestTabSwitch(tabName, subTabName) {
        ...
        if (tabName === 'financials') {
            if (['credit', 'pnl', 'money', 'cash', 'tax_ledger', 'expenses'].includes(subTabName)) {
    ...
    async fetchRealData(options = {}) {
        ...
        if (includeLookups) {
            ...
            const [taxesData, prodData, journalsData] = await Promise.all([
                ...
                this.orm.searchRead("product.product", productDomain, ["id", "name", "display_name", "uom_id", "standard_price", "product_tmpl_id"]),
